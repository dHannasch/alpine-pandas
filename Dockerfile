...
RUN apk add --no-cache --virtual g++ openblas-dev lapack-dev pcc-libs-dev freetype-dev pkgconfig gfortran \
    && ln -s /usr/include/locale.h /usr/include/xlocale.h \
    && pip install --no-cache-dir scipy \
    && python -c "import scipy" \
    && apk del --no-cache g++ openblas-dev lapack-dev pcc-libs-dev freetype-dev pkgconfig gfortran \
